...
Ha ${\mathbf{F}}$ nem lenne konstans, akkor legyenek ezek a vektorok elég kicsik abban az értelemben, hogy ${\mathbf{F}}$ már konstansnak tekinthető a vektorok által kijelölt térfogatban, ${\mathbf{x}}' = {\mathbf{Fx}}$, ${\mathbf{y}}' = {\mathbf{Fy}}$ és ${\mathbf{z}}' = {\mathbf{Fz}}$. Ekkor a transzformáció után a vektorok alakja:
\[{\mathbf{x}}' = \left( {\begin{array}{*{20}{c}}
  {1 + {\partial _x}{u_x}} \\ 
  {{\partial _x}{u_y}} \\ 
  {{\partial _x}{u_z}} 
...
\paragraph{Példa}
Legyen a testünk $A$ és $B$ pontjainak koordinátáira ${{\mathbf{r}}_A} = \left( { - 3, - 0.5} \right)$ és ${{\mathbf{r}}_B} = \left( { - 0.5, - 0.5} \right)$, valamint $\Delta {\mathbf{r}} = {{\mathbf{r}}_B} - {{\mathbf{r}}_A}$! Legyenek az új koordináták
\[\begin{aligned}
  x' &  = 2 \cdot x \\ 
  y' &  = 3 \cdot y \\ 
  z' &  = 1 \cdot z'. \\ 
...
\end{array}} \right){\mathbf{r}},
\end{equation}
amely felírható egy vektoriális szorzásként,
...
\end{equation}
azaz egy infinitezimális forgatás (azaz $\Delta {\mathbf{\varphi }} = \left( {0,0,\Delta \varphi } \right)$ nagyon kicsi), ahol $\Delta {\mathbf{\varphi }}$ értéke helytől független. A hozzá tartozó disztorziót kifejezhetjük a disztrozió definíciójából és a vektoriális szorzás indexes alakjának felhasználásával:
...
vagyis
\[\left\{ {{{\left( {{{\mathbf{\beta }}^f}} \right)}^T} =  - {{\mathbf{\beta }}^f}} \right\} \Leftrightarrow \left\{ {{{\mathbf{\beta }}^{f,{\text{asz}}}} = {{\mathbf{\beta }}^f}} \right\} \Leftrightarrow \left\{ {{{\mathbf{\beta }}^{f,{\text{sz}}}} = 0} \right\}.\]
Azt látjuk tehát, hogy egy homogén elforgatás disztorziója tisztán antiszimmetrikus, így a deformáció egzaktul 0, ha a forgatás kicsi. \Aref{eq:inf_forgat_u} egyenletből, annak deriváltjából egyből látható a disztorzió indexes izmozás nélkül is, és az is látszik belőle, hogy a szimmetrikus része 0.
...
Később látni fogjuk, hogy minden konstans deformációs gradiensű transzformáció felírható egy nyújtás és egy forgatás egymásutánjaként. Praktikus tehát a forgatást és nyújtást alapul vennünk, amelyekre már ismerjük ${\mathbf{F}}$-et és ${\mathbf{\varepsilon }}$.
...
Infinitezimális nyírások során a ${\mathbf{\beta }}$ vagy ${\mathbf{F}}$ főátlón kívüli nem $0$ elem értékét vesszük infinitezimálisan kicsinek. A megfelelő koordinátarendszerben az egytengelyű, egyszerű nyírásnál az ${\mathbf{F}}$ főátlójában eleve $1$-esek voltak, tiszta nyírásnál pedig azért lesznek szintén $1$-esek, mert az $1$-hez képest másodrendben kicsi tagok jelennének csak meg, amit elhanyagolunk. 
