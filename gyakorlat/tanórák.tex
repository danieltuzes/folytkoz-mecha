...
\section{Merev testek egyensúlya}
...
\end{itemize}
Így a kicsiny testre ható eredő erő e kettő fajta erőnek az összege, az egyiket a felületre, a másikat a térfogatra kell összegezni,
...
\end{equation}
amelyek közül a felületi arányos a kicsi felületi elemmel,
...
a térfogati pedig a térfogat nagyságával,
...
\[\int_V {{\mathbf{f}}\left( {\mathbf{r}} \right)dV}  + \int_{\partial V} {\hat \sigma \left( {\mathbf{r}} \right)d{\mathbf{A}}}  = \int_V {\rho \left( {\mathbf{r}} \right){\mathbf{\ddot r}}dV} ,\]
amely a Gauss-Ostrogradsky-féle divergencia tétel segítségével
\[\int_V {{\mathbf{f}}\left( {\mathbf{r}} \right)dV}  + \int_V {{\text{div}}\left( {\hat \sigma \left( {\mathbf{r}} \right)} \right)dV}  = \int_V {\rho \left( {\mathbf{r}} \right){\mathbf{\ddot r}}dV} .\]
Ha ez igaz minden térfogategységre, akkor az integrálást elhagyhatjuk, és igaz magára az argumentumokra is,
\begin{equation}
{\mathbf{f}}\left( {\mathbf{r}} \right) + {\text{div}}\left( {\hat \sigma \left( {\mathbf{r}} \right)} \right) = \rho \left( {\mathbf{r}} \right){\mathbf{\ddot r}}.
\end{equation}
\subsection{Lehajlás}
Négyzet keresztmetszetű gerenda lehajlásához határozzuk meg a másodrendű nyomatékát, ha az oldalak párhuzamosak az erővel, illetve ha azzal $45^\circ$-os szöget zárnak be! Mekkora egy üreges négyzetes hasábra, tömör hengerre és mekkora egy csőre?
\end{document}